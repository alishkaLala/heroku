class Ticket < ActiveRecord::Base
  ...
  def self.search needed_status=1, needed_template=nil 
  ...
  def self.seach_by_ticket_status needed_status
    case needed_status 
      when 1 #unassigned tickets (status = unassigned)
        Ticket.where("ticket_status_id = 1") 
      when 2 #open ticket = not close (close = complated or canseled)
        Ticket.where("ticket_status_id NOT IN (4,5)") 
      when 3 #on hold tickets
        Ticket.where("ticket_status_id = 3") 
      when 4 #close tickets
        Ticket.where("ticket_status_id IN (4,5)") 
      else #all ticket 
        Ticket.all
      end      
  end
